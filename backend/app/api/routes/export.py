...
from app.services import backup_service, csv_service
...
def export_csv(db: Session = Depends(get_db)) -> Response:
    ...
    return Response(
        content=csv_text,
        media_type="text/csv",
        headers={"Content-Disposition": 'attachment; filename="diecast-collection.csv"'},
    )
...
def export_backup(db: Session = Depends(get_db)) -> Response:
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="diecast-backup.json"'},
    )
